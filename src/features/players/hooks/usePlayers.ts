...
export const usePlayers = (): UsePlayersReturn => {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      const { data: players, error } = await supabase.from("players").select("*").order("number", { ascending: true });
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            ...
            setPlayers((prev) =>
              prev.map((player) => (player.id === payload.new.id ? (payload.new as Player) : player))
            );
